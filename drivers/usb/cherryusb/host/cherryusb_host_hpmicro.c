 ...
 *
 */
#define DT_DRV_COMPAT hpmicro_hpm_cherryusb_host
...
static void cherryusb_host_hpm_isr(uint32_t base)
{
	if (g_usbhost_bus[0].hcd.reg_base == base) {
		USBH_IRQHandler(0);
	} else {
		USBH_IRQHandler(1);
	}
...
DT_INST_FOREACH_STATUS_OKAY(CHERRYUSB_HOST_DEFINE)
